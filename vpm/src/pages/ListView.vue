  ...
  <div class="container">
    ...
    <teleport to="#popup">
      <ModalWin v-if="showModal" @close="closeModal" @delete="deleteTodo">
        <template v-slot:title>주의 !</template>
        <template v-slot:body>해당 내용을 삭제하시겠습니까?</template>
      </ModalWin>
    </teleport>
  </div>
...
export default {
  ...
  setup() {
    ...
    const getInfo = (_page = 1) => {
      ...
      fetch(
        `http://jkr1230.dothome.co.kr/data_read.php?page_now=${page_now.value}&data_count=${data_count}`
      )
        .then((res) => res.json())
        .then((data) => {
          // 배열데이터를 보관한다.
    ...
    const deleteTodo = () => {
      // console.log(_id);
      fetch(`http://jkr1230.dothome.co.kr/data_delete.php?id=${deleteId.value}`)
        .then((res) => res.json())
        .then((data) => {
          // console.log(data);
          // 목록갱신
    ...
    const toggleTodo = (_obj) => {
      ...
      fetch(
        `http://jkr1230.dothome.co.kr/data_update.php?id=${_obj.id}&title=${_obj.title}&body=${_obj.body}&complete=${_obj.complete}`
      )
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
    ...
    // 전체 데이터 수 받아오기
    const getTotal = () => {
      fetch(`http://jkr1230.dothome.co.kr/data_total.php`)
        .then((res) => res.json())
        .then((data) => {
          // 전체 데이터 수 갱신
    ...
    getTotal();
    // 현재 생성된 목록이 검색목록 or 일반 목록인지 구분하는 변수
    ...
    const getTotalSearch = () => {
      clearTimeout(searchTimer);
      fetch(
        `http://jkr1230.dothome.co.kr/data_total_search.php?title=${searchTxt.value}`
      )
        .then((res) => res.json())
        .then((data) => {
          // 검색어에 해당하는 데이터 개수
    ...
    const searchTodo = () => {
      fetch(
        `http://jkr1230.dothome.co.kr/data_read_search.php?page_now=${page_now.value}&data_count=${data_count}&title=${searchTxt.value}`
      )
        .then((res) => res.json())
        .then((data) => {
