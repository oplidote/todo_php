        ...
        <form @submit.prevent="onSubmit">
          ...
          <div class="form-group">
            ...
            <span
              ...
              >{{ todo.active ? "완료" : "진행중" }}</span
            >
          </div>

          <div class="btn-group" role="group" aria-label="">
            <button type="submit" class="btn btn-danger" :disabled="todoUpdate">
              확인
            </button>
...
export default {
  ...
  setup() {
    ...
    // todo originalTodo 비교 true or false 비교
    ...
    const getInfo = () => {
      fetch(
        `http://jkr1230.dothome.co.kr/data_read_id.php?id=${route.params.id}`
      )
        .then((res) => res.json())
        .then((data) => {
    ...
    const onSubmit = () => {
      ...
      fetch(
        `http://jkr1230.dothome.co.kr/data_update.php?id=${todo.id}&title=${todo.title}&body=${todo.body}&complete=${todo.complete}`
      )
        .then((res) => res.json())
        .then((data) => {
